#!/usr/bin/env python3
"""
测试使用 xhshow 调用 v1 版本的 feed API
端点：https://edith.xiaohongshu.com/api/sns/web/v1/feed
"""
...
def test_feed_v1():
    """测试 v1 feed API"""
    print("=" * 60)
    print("测试 edith.xiaohongshu.com/api/sns/web/v1/feed")
    print("=" * 60)
    ...
    # 准备请求数据（使用之前搜索到的笔记 ID）
    payload = {
        "source_note_ids": ["680e1e94000000001203f9ae"],
        "image_formats": ["jpg", "webp", "avif"],
        "extra": {"need_body_topic": "1"}
    }
    
    print(f"\n请求笔记 ID: {payload['source_note_ids'][0]}")
    ...
    try:
        ...
        print(f"\n生成的 Headers:")
        print(f"  x-s: {headers.get('x-s', 'N/A')}")
        print(f"  x-t: {headers.get('x-t', 'N/A')}")
        print(f"  x-rap-param: {headers.get('x-rap-param', 'N/A')}")
        ...
        try:
            ...
            if result.get('data') and result['data'].get('items'):
                ...
                print(f"  描述：{item.get('desc', 'N/A')[:100]}...")
                return True
            else:
                print("\n❌ 未获取到有效数据")
                if result.get('msg'):
