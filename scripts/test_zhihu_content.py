...
async def fetch_post_content_with_retry(url, max_retries=2, cookie_str=None):
    """
    带重试机制的正文获取函数
    ...
    for attempt in range(max_retries + 1):
        try:
            result = await fetch_post_content(url, cookie_str=cookie_str)
            # 如果成功获取内容，直接返回
...
async def fetch_post_content(url, cookie_str=None):
    ...
    # 清理 URL 中的追踪参数（utm_medium, utm_source 等会触发知乎反爬）
    parsed = urlparse(url)
    clean_url = urlunparse((parsed.scheme, parsed.netloc, parsed.path, '', '', ''))
    if clean_url != url:
        print(f"   清理 URL 追踪参数：{url} -> {clean_url}")
        url = clean_url
