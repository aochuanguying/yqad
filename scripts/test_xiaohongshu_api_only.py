...
"""
小红书笔记详情获取 - 纯 API 方案
使用 xhshow 生成签名，用 requests 发送请求
"""
...
def get_note_detail_api(note_id: str, cookie: str):
    """
    使用 xhshow 签名 + requests 获取笔记详情
    """
    try:
        # 清理 Cookie
        cookie = re.sub(r'[\r\n\t]+', ' ', cookie.strip())
...
def main():
    """主函数"""
    if len(sys.argv) < 3:
        print("用法：python3 test_xiaohongshu_api_only.py <note_id> <cookie>")
        print("\n示例:")
        print("  python3 test_xiaohongshu_api_only.py 69bc895d0000000021004944 'your_cookie'")
        sys.exit(1)
    ...
    print(f"\n{'='*60}")
    print(f"小红书笔记详情获取 - 纯 API 方案 (xhshow + requests)")
    print(f"{'='*60}\n")
    
    print(f"笔记 ID: {note_id}")
    print(f"Cookie: {cookie[:50]}...\n")
    
    result = get_note_detail_api(note_id, cookie)
