        ...
        try {
          ...
          if (cookie) {
            ...
            if (verifyResult.success) {
              ...
              // 新 Cookie 验证失败，说明账号被封或 session 失效
              logger.warn(`⚠️ 续期后的 Cookie 验证失败：${verifyResult.error}，标记为失效`);
              const duration = Date.now() - startTime;
              await this.storage.updateRefreshLog(duration, 'failed', `Cookie 续期后验证失败：${verifyResult.error || '无权限'}`, 'xiaohongshu');
              await this.cleanup();
