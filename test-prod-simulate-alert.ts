...
async function simulateProdAlert() {
  try {
    ...
    console.log('   - 告警手机号:', vehicleConfig.alertPhone || telecomConfig?.alertPhone || '❌ 未配置');
    console.log('   - Telecom API:', serviceConfig?.apiUrl && telecomConfig?.alertPhone ? '✅ 已配置' : '❌ 未配置');
    console.log();
    ...
    if (vehicleData.code !== 'SUCCESS') {
      console.log('   ❌ ���取车辆状态失败:', vehicleData.message);
      return;
    }
    
    const data = vehicleData.data;
    console.log('   - 在线状态:', data.isOnline ? '在线' : '离线');
    console.log('   - 异常状态:', data.isAnomaly ? '有异常 ⚠️' : '正常');
    if (data.anomalies && data.anomalies.length > 0) {
      console.log('   - 异常列表:', data.anomalies.join(', '));
    }
    console.log('   - 电池电压:', data.obd?.batteryVolt, 'V');
    console.log('   - 车门状态:', data.obd?.anyDoorOpen ? '未关 ⚠️' : '已关');
    console.log('   - 车窗状态:', data.obd?.anyWindowOpen ? '未关 ⚠️' : '已关');
    console.log('   - 设防状态:', data.obd?.isDefence ? '已设防' : '未设防 ⚠️');
    console.log('   - 位置:', data.location ? `${data.location.lat.toFixed(6)}, ${data.location.lng.toFixed(6)}` : '未知');
    console.log();
    
    // 4. 判断是否需要模拟
    if (data.isAnomaly) {
      console.log('4. ⚠️ 车辆已有异常，直接触发告警...');
      console.log('   异常列表:', data.anomalies.join(', '));
    } else {
      console.log('4. 车辆状态正常，模拟严重异常场景...');
    ...
    const result = await alertService.triggerAlert(data.anomalies, location);
